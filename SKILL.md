...
`r2_participants:` = the roles in `R2_ROSTER`, comma-separated, or `none` when R2 did not run (early
finalize via `EARLY_FINALIZE: true`, or `DISPUTED_THESES` empty after Step 6). `protagonist:` =
`yes` iff `PROTAGONIST_POSITION` was set. `tiers:` = the effective tier per dispatched agent this
run (frontmatter tier, or `MODEL_OVERRIDE` for every agent when set).
...
`tiers:` = the effective tier per dispatched agent this run (`critic=opus, champion=sonnet` per
Protocol · Model, or the `MODEL_OVERRIDE` tier for every agent when set).
